  ...
  return (
    <section className="flex flex-col gap-5">
      ...
      <div className="overflow-hidden rounded-xl border border-border">
        ...
        {units.map((u) => (
          <div key={u.id} className="flex items-center gap-3 border-b border-border px-3.5 py-2.5 text-sm last:border-0 hover:bg-hover/50"
            style={{ paddingInlineStart: `${u.level * 26 + 14}px` }}>
            <span className="flex flex-1 items-center gap-2">
              {u.level > 0 && <CornerDownLeft size={13} className="text-muted/60 -scale-x-100" />}
            ...
            <span className="whitespace-nowrap text-xs text-muted">{t('orgUnits.childUser', { children: u.childCount, users: u.userCount })}</span>
