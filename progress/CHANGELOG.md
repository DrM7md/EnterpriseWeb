...
## 2026-06-27 — تخصيص الجداول + سايدبار قابل للطي + قوائم فرعية
**ماذا:** تحسينات DataGrid وتنقّل.

- **تخصيص الأعمدة:** إظهار/إخفاء الأعمدة عبر قائمة منسدلة (`hideable`) — يُحفظ لكل جدول.
- **تبديل طريقة العرض:** جدول ↔ بطاقات، مع **حفظ آخر اختيار** (`useTablePrefs` → localStorage). مُطبَّق على المستخدمين والأدوار.
- **سايدبار قابل للطي** (مُخزَّن في التفضيلات) — أيقونات فقط عند الطي + tooltips.
- **نظام قوائم فرعية عام (submenu):** `nav.config` شجري يدعم children لأي عنصر + توسّع/طيّ + تمييز المسار النشط. مثال حقيقي: **«إدارة التوجيه التربوي»** (الموجّهون · الزيارات · التقارير) بصفحات هيكلية ومسارات.
- ترجمات AR/EN (parity 150/150).

**التحقّق:** build 0 تحذير · lint نظيف · يعمل live.
